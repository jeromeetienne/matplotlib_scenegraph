...
class RendererMeshBasicMaterial:
    ...
    def render(
        ...
        assert faces_uvs.shape == (len(geometry.indices), 3, 2), f"Expected faces_uvs to have shape {(len(geometry.indices), 3, 2)}, got {faces_uvs.shape}"
        ...
        assert len(faces_uvs) == len(geometry.indices), f"Expected faces_uvs to have {len(geometry.indices)} faces, got {len(faces_uvs)}"
